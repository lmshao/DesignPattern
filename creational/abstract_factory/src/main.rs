...
impl Table for ModernTable {
    fn put_on(&self) {
        println!(
            "🪑 Putting items on modern {} {} {} table",
            self.color, self.material, self.size
        );
    }

    fn get_info(&self) -> String {
        format!(
            "Modern Table - Material: {}, Color: {}, Size: {}",
            self.material, self.color, self.size
        )
    }
}
...
impl Sofa for ModernSofa {
    fn lie_on(&self) {
        println!(
            "🛋️ Lying on modern {} {} sofa with {} seats",
            self.color, self.material, self.seats
        );
    }

    fn get_info(&self) -> String {
        format!(
            "Modern Sofa - Material: {}, Color: {}, Seats: {}",
            self.material, self.color, self.seats
        )
    }
}
...
impl Chair for VictorianChair {
    fn sit_on(&self) {
        let carving_desc = if self.carvings {
        ...
        println!(
            ...
            self.color, self.material, carving_desc
        );
    }
    ...
    fn get_info(&self) -> String {
        ...
        format!(
            "Victorian Chair - Material: {}, Color: {}, {}",
            self.material, self.color, carving_desc
        )
    }
}
...
impl Table for VictorianTable {
    fn put_on(&self) {
        ...
        println!(
            "🪑 Putting items on victorian {} {} {} table {}",
            self.color, self.material, self.size, carving_desc
        );
    }
    ...
    fn get_info(&self) -> String {
        ...
        format!(
            "Victorian Table - Material: {}, Color: {}, Size: {}, {}",
            self.material, self.color, self.size, carving_desc
        )
    }
}
...
impl Sofa for VictorianSofa {
    fn lie_on(&self) {
        ...
        println!(
            "🛋️ Lying on victorian {} {} sofa with {} seats {}",
            self.color, self.material, self.seats, carving_desc
        );
    }
    ...
    fn get_info(&self) -> String {
        ...
        format!(
            "Victorian Sofa - Material: {}, Color: {}, Seats: {}, {}",
            self.material, self.color, self.seats, carving_desc
        )
    }
}
...
impl FurnitureFactory for ModernFurnitureFactory {
    ...
    fn create_sofa(&self, material: String, color: String, seats: u32) -> Box<dyn Sofa> {
        println!(
            "🏭 Modern factory creating sofa: {} {} with {} seats",
            material, color, seats
        );
...
impl FurnitureFactory for VictorianFurnitureFactory {
    fn create_chair(&self, material: String, color: String) -> Box<dyn Chair> {
        println!(
            "🏭 Victorian factory creating chair: {} {}",
            material, color
        );
    ...
    fn create_table(&self, material: String, color: String, size: String) -> Box<dyn Table> {
        println!(
            "🏭 Victorian factory creating table: {} {} {}",
            material, color, size
        );
    ...
    fn create_sofa(&self, material: String, color: String, seats: u32) -> Box<dyn Sofa> {
        println!(
            "🏭 Victorian factory creating sofa: {} {} with {} seats",
            material, color, seats
        );
...
impl FurnitureManufacturer {
    fn new() -> Self {
        ...
        factories.insert(
            "modern".to_string(),
            Box::new(ModernFurnitureFactory) as Box<dyn FurnitureFactory>,
        );
        factories.insert(
            "victorian".to_string(),
            Box::new(VictorianFurnitureFactory) as Box<dyn FurnitureFactory>,
        );

        Self { factories }
    }

    fn create_furniture_set(
        &self,
        style: &str,
        material: String,
        color: String,
    ) -> Option<(Box<dyn Chair>, Box<dyn Table>, Box<dyn Sofa>)> {
        if let Some(factory) = self.factories.get(style) {
            println!(
                "🏭 Using {} to create furniture set",
...
fn main() {
    ...
    for (style, material, color) in furniture_orders {
        ...
        if let Some((chair, table, sofa)) =
            manufacturer.create_furniture_set(style, material.to_string(), color.to_string())
        {
            println!();
